...
namespace detail
{
    ...
    template<typename T>
    constexpr bool isPositionalArguments = std::is_same_v<std::remove_cvref_t<T>, PositionalArguments>;

    template<typename ArgClass>
    constexpr bool hasPositionalArguments()
    {
        return [&]<auto ... Ns>(std::index_sequence<Ns...>) {
            return (... || isPositionalArguments<decltype(reflect::get<Ns>(std::declval<ArgClass>()))>);
    ...
    auto argInfo(ArgClass& args)
    {
        ...
        if(std::is_same_v<std::remove_cvref_t<decltype(reflect::get<N>(args))>, PositionalArguments>)
...
template<typename T>
concept argument_list = std::ranges::random_access_range<T> && std::is_convertible_v<std::ranges::range_value_t<T>, std::string_view>;

template<typename ArgClass>
class Parser
{
public:
    explicit Parser(ArgClass& args)
     : m_args{args}
    {
    }
    ...
    void parse(const ArgumentList& arguments)
    {
        ...
        auto ARG_INFOS = [&]<auto ... Ns>(std::index_sequence<Ns...>) {
            return std::make_tuple(
                detail::argInfo<Ns, std::remove_cvref_t<decltype(reflect::get<Ns>(m_args))>>(m_args)...
            );
        }(std::make_index_sequence<reflect::size<ArgClass>()>());

        constexpr bool ACCEPTS_POSITIONAL = detail::hasPositionalArguments<ArgClass>();
        ...
        for(std::size_t i = 0; i < std::size(arguments); ++i)
        {
            ...
            bool found = std::apply([&](auto& ... infos){ return (... || tryParam(infos)); }, ARG_INFOS);
            if(!found)
...
std::vector<std::string> serialize(const ArgClass& args)
{
    ...
    {
        const PositionalArguments* positional = detail::getPositionalArguments(args);
