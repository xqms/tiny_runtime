...
namespace
{
    ...
    public:
        MappedFile(const char* path)
        {
            ...
            if(fd < 0)
                throw std::runtime_error{fmt::format("Could not open file {}: {}", path, strerror(errno))};

            void* data = mmap(nullptr, m_size, PROT_READ, MAP_PRIVATE, fd, 0);
            if(data == MAP_FAILED)
            {
                close(fd);
                throw std::runtime_error{fmt::format("Could not mmap file {}: {}", path, strerror(errno))};
            }
...
public:
    Private()
    {
        ...
        if(!strncmp(h5->magic, MAGIC_LIBC5, MAGIC_LIBC5_LEN))
        {
            ...
            std::size_t padding = (-(uintptr_t)ptr) & (__alignof__(struct header_libc6) - 1);
            ptr += padding;
        }
    ...
    std::vector<LDCache::Entry> resolve(const std::span<const char* const>& libraries) const
    {
...
std::vector<LDCache::Entry> LDCache::resolve(const std::span<const char* const>& libraries) const
{
