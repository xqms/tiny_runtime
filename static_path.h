...
constexpr StaticPath<LengthA + 1 + LengthB> operator/(const StaticPath<LengthA>& a, const StaticPath<LengthB>& b)
{
...
constexpr StaticPath<LengthA + 1 + LengthBPlusOne - 1> operator/(const StaticPath<LengthA>& a, const char (&b) [LengthBPlusOne])
{
...
template<std::size_t Length>
auto format_as(const StaticPath<Length>& path)
{
    return path.data;
}
